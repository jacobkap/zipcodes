...
zip_scraper <- function(){
  ...
  for (i in state.name) {
    if (i == "New Hampshire") {
      zipcode_site <-
        xml2::read_html("http://www.zipcodestogo.com/New%20Hampshire/")
    }
    else if (i != "New Hampshire") {
      zipcode_site <- xml2::read_html(paste("http://www.zipcodestogo.com/",
                                      i, "/", sep = ""))
    }
    zipcode <- zipcode_site %>%
      rvest::html_nodes("#leftCol tr+ tr td:nth-child(1) a") %>%
      rvest::html_text()

    city <- zipcode_site %>%
      rvest::html_nodes("#leftCol tr~ tr+ tr td:nth-child(2)") %>%
      rvest::html_text()

    county <- zipcode_site %>%
      rvest::html_nodes("#leftCol tr~ tr+ tr td:nth-child(3)") %>%
      rvest::html_text()
  ...
  dc_site <- xml2::read_html(paste("http://www.zillow.com/browse/homes/",
                                 "dc/district-of-columbia-county/",
                                 sep = ""))

  dc_zipcode <- dc_site %>%
    rvest::html_nodes(".zsg-sm-1-1 a") %>%
    rvest::html_text()
